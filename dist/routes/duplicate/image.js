...
exports.duplicateImageRouter = void 0;
const ImageRepository_1 = require("../../api/repositories/ImageRepository");
const duplicateImageRouter = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    let duprs = yield ImageRepository_1.ImageRepository.aggregate([
        {
            $match: {
    ...
    for (let i = 0; i < arr.length; i++) {
        let el = arr[i];
        let allDocs = yield ImageRepository_1.ImageRepository.find({
            where: {
        ...
        allDocs.shift();
        allDocs.forEach((el) => {
            ImageRepository_1.ImageRepository.delete(el._id);
        });
    }
    res.send({ arr });
});
exports.duplicateImageRouter = duplicateImageRouter;
