...
exports.duplicateItemRouter = void 0;
const ItemRepository_1 = require("../../api/repositories/ItemRepository");
const duplicateItemRouter = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    let duprs = yield ItemRepository_1.ItemRepository.aggregate([
        {
            $match: {
    ...
    for (let i = 0; i < arr.length; i++) {
        let el = arr[i];
        let allDocs = yield ItemRepository_1.ItemRepository.find({
            where: {
        ...
        allDocs.shift();
        allDocs.forEach((el) => {
            ItemRepository_1.ItemRepository.delete(el._id);
        });
    }
    res.send({ arr });
});
exports.duplicateItemRouter = duplicateItemRouter;
