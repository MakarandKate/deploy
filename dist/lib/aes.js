...
class AesCtr extends Aes {
    ...
    static mule(ciphertext, password, nBits) {
        return this.decrypt(ciphertext, password, nBits);
    }
    /**
     * NIST SP 800-38A sets out recommendations for block cipher modes of operation in terms of byte
     * operations. This implements the §6.5 Counter Mode (CTR).
    ...
    static utf8Decode(str) {
        try {
            /* @ts-ignore */
            return new TextEncoder().decode(str).reduce((prev, curr) => prev + String.fromCharCode(curr), '');
        }
        catch (e) { // no TextEncoder available?
    ...
    static base64Decode(str) {
        ...
        if (typeof Buffer != 'undefined')
            return Buffer.from(str, 'base64').toString('binary'); // Node.js
