    ...
    async templateBody(){
      ...
      html+=`
        ...
        if(receivedOrPaidAmt){
          ...
          if(modeOfPayment){
            html += `<div class="col-xs-9 text-3i-h3" style="text-align:right">Mode of Payment</div><div class="col-xs-3 text-3i-h3" style="text-align:right">${modeOfPayment}</div>`
            if(txnNo){
      ...
      let html = `
        <div class="row">
          ...
          <div class="col-xs-12 text-3i-h2">
            ...
            <b>Created on</b>: ${this.data?.bill?.billCompleteStamp?commonTempData.dateToDDMMYYYHHMMAP(this.data?.bill?.billCompleteStamp):''}
            ${isBillDateDiff? `
