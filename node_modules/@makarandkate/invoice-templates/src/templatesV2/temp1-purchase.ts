...
/*       if(this.data.deliveryLocation || this.data.deliveryState){
    ...
    //Naming this div as div2, close it after applying conditions
      ...
      // if(this.data?.isMoneyIn && this.data?.moneyInOut){
      //   html += this.commonRightSideFormatForPartyDetails("Receipt No", this.data?.moneyInOut?.billNo);
      //   html += this.commonDateFormat('Date', this.data?.moneyInOut?.dateStamp);

      // }
      // if(this.data?.isMoneyOut && this.data?.moneyInOut){
      //   html += this.commonRightSideFormatForPartyDetails("Receipt No", this.data?.moneyInOut.billNo);
      //   html += this.commonDateFormat('Date', this.data?.moneyInOut.dateStamp);

      // }
     
      let refInvoiceNumber= this.data?.bill?.billNo;
      html += this.commonRightSideFormatForPartyDetails('Purchase No', this.data?.bill?.billNo);
    ...
    footerAmountCalculation(){
      ...
      if(PaidAmt){
        html +=this.commonAmountCalculationFormatForFooter(receivedOrPaid, commonTempData.unitAdjuster(PaidAmt||0));
        if(modeOfPayment){
      ...
      let balanceAmt = ((this.data?.bill?.totalAmount||0) - PaidAmt); //10
      if(balanceAmt>0.01){
      ...
      if(this.data?.settings.showPreviousAmount && this.data?.bill?.partyData?.ledgerCredit != null && this.data?.bill?.partyData?.ledgerCredit != undefined){
        html +=this.commonAmountCalculationFormatForFooter('Previous Balance', commonTempData.unitAdjuster(this.data?.bill?.partyData?.ledgerCredit||0));
        if(balanceAmt>0){
          html +=this.commonAmountCalculationFormatForFooter('Total Balance', commonTempData.unitAdjuster((this.data?.bill?.partyData?.ledgerCredit||0) - balanceAmt));
        }
      }
